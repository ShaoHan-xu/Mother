...
public class RoundedBitmapDisplayer implements BitmapDisplayer {
	...
	public static class RoundedDrawable extends Drawable {
		...
		public RoundedDrawable(Bitmap bitmap, int cornerRadius, int margin) {
			...
			if (h > w) {
				int top = (h - w) / 2 + margin;
				int bottom = h - top;
				mBitmapRect = new RectF(margin, top,
						bitmap.getWidth() - margin, bottom);
			} else {
				int left = (w - h) / 2 + margin;
				int right = w - left;
				mBitmapRect = new RectF(left, margin, right, bitmap.getHeight()
						- margin);
			}
